...
The Grammar tab is the word's anatomy per language (language/grammar.py,
shared with the card's Breakdown field), under the same laziness contract as
Examples — Arabic's first analysis loads a 400 MB database.
...
from ..language import examples, grammar, pronounce
...
class WordPopup(QWidget):
    ...
    def _fetch_grammar(self, req, surface, lemma):
        ...
        try:
            content, err = grammar.anatomy_html(surface, lemma), ""
        except Exception:
